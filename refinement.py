...
ori_pdb_file_path = "./datasets/AlphaFold_model_PDBs/" + args.proteinname  + ".pdb"
...
class RewardCal:
    def __init__(
            ...
            metrics_list: str,
            esm_model: str,
            pretrained_model: str,  # momst
            pretrained_model_tokenizer:str,  # momst
            seq_len,
            repeat_num,
            device,
            run_name="",
        ...
        # initialize the pretrained_model for likelihood calculation
        self.pretrained_model = pretrained_model
        self.pretrained_model_tokenizer = pretrained_model_tokenizer
        self.seq_len = seq_len
        self.repeat_num = repeat_num
        self.device = device

    def metrics_cal(self, metrics_name, ori_pdb_file=None, gen_pdb_file=None, folding_results=None, sequence_string=None, tokenized_sample=None, protein_idx=0, save_pdb=False, pdb_raw =None, pretrained_model=None, tokenizer=None,seq_len=None, repeat_num=None, device=None):
        # all_results = []
        reward_dict = {}
        for metric in metrics_name:
            ...
            elif metric == 'hydrophilic':
                r = pdb_to_hydrophilic_score(gen_pdb_file if save_pdb else StringIO(gen_pdb_file))
            ...
            reward_dict[metric] = r
        return reward_dict
        # return all_results

    # ✅ MODIFIED: Now returns 3D normalized rewards instead of scalar aggregate
    def reward_metrics(self, protein_name , mask_for_loss, S_sp, ori_pdb_file, save_pdb=False, add_info = ""):
        ...
        for _it, ssp in enumerate(S_sp):  # transform the sequence to string
        ...
        record_reward_multi = []  # 🆕 NEW: list of [hydro_norm, surface_norm, plddt_norm]
        record_reward_all = []
        record_reward_agg = []
        all_reward_dicts = []
        ...
        for _it, pdb in enumerate(pdbs):
            ...
            sequence_string = esm_input_data[_it]
            sequence_tokenized = S_sp[_it]

            # calculate metrics   !!!
            reward_dict = self.metrics_cal(  # all_reward
                metrics_name=self.metrics_name,
                gen_pdb_file=pdb_path,
                ori_pdb_file=ori_pdb_file,
                folding_results=output,
                sequence_string=sequence_string, # momst
                tokenized_sample=sequence_tokenized,  # momst
                protein_idx=_it,
                save_pdb=save_pdb,
                pdb_raw = pdb,
                pretrained_model=self.pretrained_model,  # momst
                tokenizer=self.pretrained_model_tokenizer,  # momst
                seq_len=self.seq_len,  # momst
                repeat_num=self.repeat_num,  # momst
                device=self.device  # momst
            )

            # DYNAMIC: Normalize rewards based on self.metrics_name and config
            multi_reward = []
            for metric in self.metrics_name:
                raw = reward_dict[metric]
                multi_reward.append(float(raw))
            record_reward_multi.append(multi_reward)

            # Keep old aggregate for logging (optional)
            agg = sum(v * w for v, w in zip(multi_reward, self.metrics_list))
            record_reward_agg.append(agg)
            all_reward_dicts.append(reward_dict)

            # 🆕 RETURN: multi_reward is now primary output
            # return record_reward, record_reward_agg, 0.0
        return record_reward_multi, record_reward_agg, all_reward_dicts, 0.0

if __name__ == "__main__":
    # # Handle resume path
    # resume_path = args.resume_from_trajectory
    # if resume_path is not None:
    #     # If resume path is provided, use it as the folder path
    #     folder_path = f"log/"
    # else:
    ...
    #
    # if os.path.exists(folder_path) and resume_path is None:
    #     warnings.warn(f"Result folder already exists: {folder_path}", RuntimeWarning)
    # os.makedirs(folder_path, exist_ok=True)

    folder_path = f"log/" + str(current_datetime) + args.decoding + "_" + str(args.metrics_name) + "_" + str(
        args.metrics_list) + "_" + str(args.proteinname)
    ...
    new_reward_model = RewardCal(  # model init for reward calculation
        metrics_name=args.metrics_name,
        metrics_list=args.metrics_list,
        esm_model=args.esm_model,
        pretrained_model=model,  # momst
        pretrained_model_tokenizer=tokenizer,  # momst
        seq_len=seq_len,  # momst
        repeat_num=repeat_num,  # momst
    ...
    if args.decoding == 'random':
        tokenized_sample, generated_sequence = generate_oaardm(model, tokenizer, seq_len, batch_size=repeat_num,
                                                                device=device)
    elif args.decoding == 'SVDD': # Baseline, soft value
        ...
        tokenized_sample, generated_sequence = generate_oaardm_reward_metrics_edit_momost(
            ...
            iteration = args.iteration, 
            device=device,
            reward_metrics_name=args.metrics_name,
            rewards_weights=[float(x) for x in args.metrics_list.split(",")],
    ...
    #         mask_for_loss= mask_for_loss,
    #         repeat_num=repeat_num,
    #         candidate=duplicate,  # the number of generated candidates, default 20
    #         edit_seqlength = edit_seqlength,
    #         folder_path = folder_path,
    #         iteration = args.iteration,
    #         device=device,
    ...
    test_reward_model = RewardCal(  # 初始化奖励模型，init reward model
        metrics_name=args.metrics_name ,
        metrics_list=args.metrics_list ,
        esm_model=args.esm_model,
        pretrained_model=model,  # momst
    ...
    # reward_name_list = new_reward_model.metrics_name  # momst

    likelihooed_reward = likelihood(model, tokenizer,seq_len, tokenized_sample, repeat_num, device)

    cur_reward_before, cur_reward_agg, _, _ = test_reward_model.reward_metrics(
    ...
    # cur_reward_before, cur_reward_agg, _ = new_reward_model.reward_metrics(
    #     protein_name=protein_name,
    #     mask_for_loss=mask_for_loss,
    #     S_sp=tokenized_sample,  # !!!
    #     ori_pdb_file=ori_pdb_file_path,
    ...
    reward_array = np.array(cur_reward_before)
    # negate_indices = [i for i, name in enumerate(reward_name_list) if name in ['hydrophobic', 'globularity']]
    # reward_array[negate_indices] = -reward_array[negate_indices]

    df = pd.DataFrame(reward_array.transpose(), columns= reward_name_list)
    ...
    # >>>>>>>>>> Modify the saving method: write to the same CSV, divided into two parts <<<<<<<<<<
    output_file = folder_path + "/output.csv"
